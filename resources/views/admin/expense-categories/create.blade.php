    ...
    <div class="w-full md:w-7/12 px-3">
      <div class="bg-white shadow-xl rounded-2xl p-6">
        ...
        <div class="flex items-center mb-6" style="padding-bottom:1rem; border-bottom:1px solid #f1f5f9;">
          <div>
            <h2 class="text-lg font-semibold">✨ Tambah Kategori Pengeluaran</h2>
            <p style="font-size:0.78rem; color:#94a3b8; margin-top:2px;">Buat kategori baru untuk mengelompokkan pengeluaran</p>
          </div>
          <a href="{{ route('admin.expense-categories.index') }}"
             class="ml-auto px-4 py-2 text-xs font-bold text-black bg-success-500 rounded-lg btn-back">
            ← Kembali
          </a>
        </div>
        ...
        <div class="w-full mb-4">
          ...
          <input type="text" name="name" value="{{ old('name') }}"
                 placeholder="cth: Bahan Baku, Listrik, Transport..."
                 class="w-full px-3 py-2 border rounded-lg"
                 id="input-name" required>
          <p style="font-size:0.72rem; color:#94a3b8; margin-top:4px;">Nama kategori akan ditampilkan di daftar pengeluaran</p>
        </div>
    ...
    <div class="w-full md:w-5/12 px-3">
      <div class="bg-white shadow-xl rounded-2xl p-6">
        ...
        <div style="background:#f5f3ff; border-radius:12px; padding:20px; text-align:center; margin-bottom:16px;">
          ...
          <p id="preview-name" style="font-weight:700; font-size:0.875rem; color:#7c3aed;">Nama Kategori</p>
          <p id="preview-desc" style="font-size:0.75rem; color:#94a3b8; margin-top:4px;">Deskripsi akan muncul di sini</p>
        </div>

        <p style="font-size:0.72rem; color:#94a3b8; text-align:center;">Begini tampilan kategori di daftar pengeluaran</p>
...
document.getElementById('input-desc').addEventListener('input', function() {
  const v = this.value.trim();
  document.getElementById('preview-desc').textContent = v || 'Deskripsi akan muncul di sini';
});
</script>
