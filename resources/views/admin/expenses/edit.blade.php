...
<div class="w-full p-6 mx-auto">
  <div class="flex flex-wrap -mx-3">
    ...
    <div class="w-full md:w-8/12 px-3">
      <div class="bg-white shadow-xl rounded-2xl p-6">
        ...
        <div class="flex items-center mb-6" style="padding-bottom:1rem; border-bottom:1px solid #f1f5f9;">
          <div>
            <h2 class="text-lg font-semibold">✏️ Edit Pengeluaran</h2>
            <p style="font-size:0.78rem; color:#94a3b8; margin-top:2px;">Perbarui data pengeluaran</p>
          </div>
        ...
        <div class="w-full mb-4">
          ...
          <input type="number" step="1" name="amount" value="{{ old('amount', $expense->amount) }}"
                 class="w-full px-3 py-2 border rounded-lg" required>
        </div>
        ...
        <div class="w-full mb-5">
          ...
          <textarea name="notes" class="w-full px-3 py-2 border rounded-lg" rows="3">{{ old('notes', $expense->notes) }}</textarea>
        </div>
    ...
    <div class="w-full md:w-4/12 px-3">
      <div class="bg-white shadow-xl rounded-2xl p-6">
        <p class="text-xs font-bold mb-4">📋 Info</p>
        <div style="font-size:0.78rem; display:flex; flex-direction:column; gap:10px;">
          <div style="display:flex; justify-content:space-between; padding-bottom:8px; border-bottom:1px solid #f1f5f9;">
            <span style="color:#94a3b8;">ID</span>
            <span style="font-weight:600;">#{{ $expense->id }}</span>
          </div>
          <div style="display:flex; justify-content:space-between; padding-bottom:8px; border-bottom:1px solid #f1f5f9;">
            <span style="color:#94a3b8;">Dibuat</span>
            <span style="font-weight:600;">{{ $expense->created_at->format('d M Y') }}</span>
          </div>
          <div style="display:flex; justify-content:space-between; padding-bottom:8px; border-bottom:1px solid #f1f5f9;">
            <span style="color:#94a3b8;">Diperbarui</span>
            <span style="font-weight:600;">{{ $expense->updated_at->format('d M Y') }}</span>
          </div>
          <div style="display:flex; justify-content:space-between;">
            <span style="color:#94a3b8;">Jumlah</span>
            <span style="font-weight:700; color:#7c3aed;">Rp {{ number_format($expense->amount, 0, ',', '.') }}</span>
          </div>
        </div>
      </div>
    </div>
