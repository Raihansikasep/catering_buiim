  ...
  <div class="flex-none w-full max-w-full px-3">
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white shadow-xl rounded-2xl">
      ...
      <div class="p-6 pb-0 mb-4 flex justify-between items-start">
        <div>
          <h6 class="text-lg font-semibold">💸 Daftar Pengeluaran</h6>
          <p style="font-size:0.78rem; color:#94a3b8; margin-top:2px;">Kelola semua data pengeluaran operasional</p>
        </div>
          ...
          <table class="items-center w-full mb-0 align-top border-collapse text-slate-600">
            ...
            <tbody>
              ...
              <tr class="border-b">
                ...
                <td class="px-6 py-3">
                  <span style="display:inline-block; padding:2px 10px; border-radius:999px; font-size:0.72rem; font-weight:600; background:#f3f0ff; color:#7c3aed;">
                    {{ $expense->category->name ?? '-' }}
                  </span>
                </td>
                <td class="px-6 py-3">{{ Str::limit($expense->description, 50) }}</td>
                <td class="px-6 py-3 font-medium">Rp {{ number_format($expense->amount, 0, ',', '.') }}</td>
                <td class="px-6 py-3">{{ \Carbon\Carbon::parse($expense->expense_date)->format('d M Y') }}</td>
                <td class="px-6 py-3" style="color:#94a3b8;">{{ Str::limit($expense->notes, 40) ?? '-' }}</td>

                <td style="text-align:center;">
                    <div class="action-group" style="justify-content:center;">
                        <a href="{{ route('admin.expenses.edit', $expense->id) }}" class="btn btn-primary btn-sm">✏️ Edit</a>
